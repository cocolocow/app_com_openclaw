...
type Step = "input" | "waiting" | "confirmed" | "rejected" | "expired";
...
export function Onboarding() {
  ...
  // Lance le polling quand on a un pendingId
  useEffect(() => {
    if (!pendingId || step !== "waiting") return;
    stopPoll();

    pollRef.current = setInterval(async () => {
      const res = await pollConfirmation(pendingId);
      if (res.status === "confirmed" && res.token) {
        stopPoll();
        setStep("confirmed");
        setTimeout(() => {
          setConfig({ token: res.token!, mDNS: "openclaw", baseUrl: "" });
        }, 1200);
      } else if (res.status === "rejected") {
        stopPoll(); setStep("rejected");
  ...
  const handlePair = useCallback(async () => {
    ...
    if (result.token) {
      setConfig({ token: result.token, mDNS: "openclaw", baseUrl: "" });
    }
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-8 bg-bg-primary text-center">
      <div className="mb-8">
        ...
        <p className="text-text-secondary text-sm">
          Appuyez sur <span className="text-green-400 font-bold">OUI</span> sur l'écran tactile de votre Nodi
        </p>
      </div>
      <div className="w-full max-w-sm">
        ...
        <button onClick={reset} className="text-xs text-text-secondary underline">
          Annuler
        </button>
      </div>
    </div>
    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-8 bg-bg-primary">
      ...
      <div className="w-full max-w-sm space-y-4">
        <p className="text-text-secondary text-xs text-center">
          Enter the code shown on your Nodi screen
        </p>
        <input
          type="text"
          inputMode="text"
          ...
          value={pairingCode}
          onChange={(e) =>
            setPairingCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 6))
          }
          maxLength={6}
